...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="한국어 위키피디아 코퍼스 생성 (HuggingFace streaming)"
    )
    ...
    if args.upload:
        ...
        print(
            f"   uv run hf upload bllm-study/kowiki-corpus {args.data_dir}/wiki_corpus.txt --repo-type dataset"
        )
        print(
            f"   uv run hf upload bllm-study/kowiki-corpus {args.data_dir}/wiki_clean.jsonl --repo-type dataset"
        )
        print()
